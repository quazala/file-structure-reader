...
describe("FileStructureReader", () => {
  ...
  test("filesByFolder returns correct files for each folder", () => {
    expect(collector.filesByFolder(tempDir)).toContain(
      path.join(tempDir, "file1.txt")
    );
    expect(collector.filesByFolder(path.join(tempDir, "folder1"))).toContain(
      path.join(tempDir, "folder1", "file2.txt")
    );
    expect(collector.filesByFolder(path.join(tempDir, "folder2"))).toContain(
      path.join(tempDir, "folder2", "file3.txt")
    );
  });
